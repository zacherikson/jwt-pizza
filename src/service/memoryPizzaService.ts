...
class MemoryPizzaService implements PizzaService {
  ...
  async getOrders(requestUser): Promise<OrderHistory> {
    return new Promise(async (resolve) => {
      ...
      if (requestUser) {
        ...
        if (user && (Role.isRole(user, Role.Admin) || user.id === requestUser.id)) {
  ...
  async getFranchise(franchiseUser: User): Promise<Franchise | null> {
    return new Promise(async (resolve) => {
      if (franchiseUser) {
        const user = await this.getUser();
        if (user != null && (Role.isRole(user, Role.Franchisee) || Role.isRole(user, Role.Admin))) {
  ...
  async createStore(franchise: Franchise, store: Store): Promise<Store> {
    return new Promise(async (resolve, reject) => {
      if (store?.name) {
        const user = await this.getUser();
        if (Role.isRole(user, Role.Franchisee) || Role.isRole(user, Role.Admin)) {
