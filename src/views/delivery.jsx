    ...
    <View title='Here is your JWT Pizza!'>
      <div className='my-4 flex max-w-xl justify-center items-center flex-col  py-8 px-4 sm:px-6 lg:px-8'>
        ...
        <div className='my-4 text-lg text-orange-200 text-start grid grid-cols-5 gap-2'>
          ...
          <div className='font-semibold text-orange-400'>pie count:</div>{' '}
          <div className='col-span-4'>{order.items.length}</div>
          <div className='font-semibold text-orange-400'>total:</div>{' '}
          <div className='col-span-4'>{order.items.reduce((a, c) => a + c.price, 0).toLocaleString()} ₿</div>
        </div>

        <div
          className={`font-thin break-all font-mono text-xs bg-slate-100 p-2 ${
            jwtPayload.message === 'valid' ? 'text-green-500' : 'text-red-500'
          }`}
        >
          {jwt}
        </div>
          ...
          <div className='w-full flex flex-col bg-white border shadow-sm rounded-xl pointer-events-auto   '>
            <div className='flex justify-between items-center py-3 px-4 border-b bg-slate-200 rounded-t-xl '>
              <h3 className='font-bold text-gray-800'>
                JWT Pizza -{' '}
                <span className={jwtPayload.message === 'valid' ? 'text-green-800' : 'text-red-800'}>
                  {jwtPayload.message}
                </span>
              </h3>
              <button
            ...
            <div className='p-4 overflow-y-auto'>
              <pre className='text-gray-800 text-xs'>{JSON.stringify(jwtPayload.payload, 0, 2)}</pre>
            </div>
